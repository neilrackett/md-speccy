...
void emul_start() {
  ...
  // ROM3 cart-bus capture ring (PIO + 32 KB DMA) BEFORE fb_init, whose
  // first fb_publish() drains it while waiting for the m68k VBL ack.
  if (commemul_init() < 0) {
  ...
  // SD card -- best effort. Folder name comes from per-app config
  // (ACONFIG_PARAM_FOLDER, default "/zx"); games + keymaps.txt live
  // there.
  FATFS fsys;
